...
#SBATCH --job-name=V2R_Large_genomes   # Job name
...
#genome=../Splitted_genome/Combined_genome_subparts.fna
#V2R_database=/scicore/home/salzburg/polica0000/Mammals_firsts_tests/Databases/Database_V2R_lungfish_newstrategy/Database_2022_V2R_vertebrates_cdhit_80.prot
#blast_database=/scicore/home/salzburg/polica0000/Mammals_firsts_tests/Databases/GPCR_plus_Olfactory_plus_Taste_receptors_vertebrates_rodentV2R.prot
#scripts_location=/scicore/home/salzburg/polica0000/Mammals_firsts_tests/Databases/Database_V2R_lungfish_newstrategy/Scripts_2022
#maximum_intron_length=30000
#number_of_thread=10
#tm_filter=TRUE
#maximum_intron_length_half=$((maximum_intron_length / 2))

# Becareful this script does not perform the blast ! This should be run before
...
#Extend all best hits by 5000bp upstream and downstream . Result file : Potential_V2R_regions.tsv
Rscript $scripts_location/Rscript_merge_filter_extend_blast_hit_lungfish.R 


#Now for each scaffold, extract only the corresponding regions and merge them, and create what I call a "V2R subgenome"

sed 's/:.*//g' Potential_V2R_regions.tsv  | sort | uniq > scaffold_list


rm V2R_subgenome.fa ; rm V2R_subgenome.fa.fai
for scaff in `cat scaffold_list` ; do 
	grep "$scaff:" Potential_V2R_regions.tsv > to_extract.tsv
	xargs samtools faidx ../Splitted_genome/Combined_genome_subparts.fna < to_extract.tsv > to_extract.fa
	grep -v ">" to_extract.fa > to_extract.txt

	sed -i "1 i\>$scaff" to_extract.txt
	fasta_formatter -i to_extract.txt -o to_extract.fa -w 60

	cat to_extract.fa >> V2R_subgenome.fa
...
cp V2R_subgenome.fa Initial_V2R_subgenome.fa
...
while [ "$current_nb_sequences" -gt "$previous_iteration_nb_sequences" ] && [ "$number_regions_blast" -gt "0" ] ; do
	...
	if test -f "V2R_subgenome.tsv" ; then xargs samtools faidx Initial_V2R_subgenome.fa < V2R_subgenome.tsv > V2R_subgenome.fa ; fi
	rm *.fai ; sed -i 's/:/-/g' V2R_subgenome.fa
	...
	for i in Splitted_db/* ; do
		...
		sbatch -W -c 4 --qos=6hours --wrap="$scripts_location/exonerate-2.2.0-x86_64/bin/exonerate -E True --showtargetgff TRUE --model protein2genome --minintron 50 --maxintron $maximum_intron_length --ryo '%tcs' Splitted_db/$file_name V2R_subgenome.fa > Exonerate_raw_results_folder/$file_name.exo.rslt ; sleep 10" &
	done
	...
	if [ "$nb_row_parsed_exonerate" -gt "0" ] ; then
		...
		for line in `cat Correct_coordinates_for_exonerate.tsv` ; do 
			...
			samtools faidx V2R_subgenome.fa $scaffold_s_e > scaffold.fa
			...
			if grep -q -i "V2R-Receptor" blastp_result ; then
				...
				#file=Genes_predictions/NC_019879.2-28438421-28440954.exonerate 
					...
					if [ `grep -c ">" Filtered_predictions/$file_name_reduced.ORF` -ge 1 ] ; then
						...
						nb_tiret=`echo "$file" | sed 's/.*\///g' | sed 's/.exonerate//g'  | grep -o "-" | wc -l`
						if [ $nb_tiret -ge 4 ] ; then extracted_scaffold_start=`echo "$file" | sed 's/.*\///g' | sed 's/.exonerate//g' | cut -f2,4 -d "-" | tr - '\n' | awk -F',' '{sum+=$1;} END{print sum;}'` ; fi 

						cds_end_extract=`grep -m1 "Target range:" $file | sed 's/^ *//g' | sed 's/Target range://g' | sed 's/ //g' | sed 's/->/ /g' | cut -f1 -d " "`
						cds_start_extract=`grep -m1 "Target range:" $file | sed 's/^ *//g' | sed 's/Target range://g' | sed 's/ //g' | sed 's/->/ /g' | cut -f2 -d " "`
						cds_coord_start=$((extracted_scaffold_start + cds_start_extract))
						cds_coord_end=$((extracted_scaffold_start + cds_end_extract - 1))
						exon_number=`grep "	exon	" $file | wc -l`
						...
						samtools faidx Initial_V2R_subgenome.fa $scaffold:$cds_coord_start-$cds_coord_end > Verification_scaffold.fa
					...
					#If not ORF found, then determinate the gene state
						...
						scaffold_length=`grep -m1 "^$scaffold	" Initial_V2R_subgenome.fa.fai | cut -f2` #extract scaffold length from .fai file
						...
						consecutive_N_nb=`samtools faidx Initial_V2R_subgenome.fa $scaffold:$extanded_start_coord-$extanded_end_coord | sed 's/n/N/g' | grep -v ">" | awk '/^>/ {printf("\n%s\n",$0);next; } { printf("%s",$0);}  END {printf("\n");}' | grep N | awk -F '[^N]+' '{for (i=1; i<=NF; i++) if ($i != "") print length($i)}' | sort -n | tail -1`
						...
						#check that there were no merge between two genes with a tblastn (number of tblastn hits should be inferior or equal to the number of exon)
						samtools faidx Initial_V2R_subgenome.fa $scaffold:$true_start_coord-$true_end_coord > Verification_scaffold.fa
						makeblastdb -in Verification_scaffold.fa -dbtype nucl
				...
				#Lets make the same steps with slight modifications for the + strand
					...
					if [ `grep -c ">" Filtered_predictions/$file_name_reduced.ORF` -ge 1 ] ; then
						...
						nb_tiret=`echo "$file" | sed 's/.*\///g' | sed 's/.exonerate//g'  | grep -o "-" | wc -l`
						if [ $nb_tiret -ge 4 ] ; then extracted_scaffold_start=`echo "$file" | sed 's/.*\///g' | sed 's/.exonerate//g' | cut -f2,4 -d "-" | tr - '\n' | awk -F',' '{sum+=$1;} END{print sum;}'` ; fi 
						cds_start_extract=`grep -m1 "Target range:" $file | sed 's/^ *//g' | sed 's/Target range://g' | sed 's/ //g' | sed 's/->/ /g' | cut -f1 -d " "`
						cds_end_extract=`grep -m1 "Target range:" $file | sed 's/^ *//g' | sed 's/Target range://g' | sed 's/ //g' | sed 's/->/ /g' | cut -f2 -d " "`
						cds_coord_start=$((extracted_scaffold_start + cds_start_extract))
						cds_coord_end=$((extracted_scaffold_start + cds_end_extract - 1))
						exon_number=`grep "	exon	" $file | wc -l`
						sed -i "s/>.*/>$scaffold-$cds_coord_start-$cds_coord_end---$exon_number\_exons/g" Filtered_predictions/$file_name_reduced.ORF
	...
	rm Initial_V2R_subgenome.fa.fai ; samtools faidx Initial_V2R_subgenome.fa 
	...
	if test -f "V2R_subgenome.tsv" ; then number_regions_blast=`grep "[0-9]" V2R_subgenome.tsv | wc -l` ; else number_regions_blast=0 ; fi
