...
regions_functionnals_or <- read.table("Coordinates_Functionnal_TAAR.txt", header=FALSE, sep="\t")
...
for (i in seq(1:nrow(Best_hits_filtered))){
	...
	if (nrow(regions_functionnals_or %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-5000) & start < curr_start)) > 0){
		...
		new_curr_start = (tail(regions_functionnals_or %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-5000) & start < curr_start) %>% dplyr::arrange(end), 1)$end) + 50
	...
	if (nrow(regions_functionnals_or %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+5000) & end > curr_end)) > 0){
		...
		new_curr_end = (head(regions_functionnals_or %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+5000) & end > curr_end) %>% dplyr::arrange(start), 1)$start) - 50
