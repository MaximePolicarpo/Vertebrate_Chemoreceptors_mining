...
for (row in 1:nrow(coordinates_to_extend)) {
  ...
  if (nrow(regions_correct_genes %>% filter(seqnames == scaffold_name) %>% filter(end >= (curr_start-extension_length) & start < curr_end)) > 0){

    new_curr_start = (tail(regions_correct_genes %>% filter(seqnames == scaffold_name) %>% filter(end >= (curr_start-extension_length) & start <= curr_start) %>% dplyr::arrange(end), 1)$end) + 100

  } else new_curr_start = curr_start-extension_length



  if (nrow(regions_correct_genes %>% filter(seqnames == scaffold_name) %>% filter(start <= (curr_end+extension_length) & end > curr_end)) > 0){

    new_curr_end = (head(regions_correct_genes %>% filter(seqnames == scaffold_name) %>% filter(start <= (curr_end+extension_length) & end >= curr_end) %>% dplyr::arrange(start), 1)$start) - 100
...
coordinates_to_extend <- 
coordinates_to_extend %>% mutate(corrected_start=if_else(
  Extanded_start <= 1,
  1,
  Extanded_start
))
...
coordinates_to_extend_test <- coordinates_to_extend %>% select(seqnames, corrected_start, Extanded_end)
colnames(coordinates_to_extend_test) <- c("seqnames", "start", "end")

coordinates_to_extend_test <- coordinates_to_extend_test %>% filter(! end <= start)
