...
#SBATCH --job-name=T1R_On   # Job name
...
#Perform tblastn using known T1R genes against the genome with an evalue of 1e-10

tblastn -query $T1R_database -db $genome -evalue 1e-10 -outfmt 6 -out T1R_vs_Genome.blastn -num_threads $number_of_thread
...
#Retain only besthit that best match to a T1R gene
...
grep "T1R-Receptor\|Taste-receptor" blastx_blast_regions.tsv | cut -f1 | sort | uniq > T1R_best_hits.txt
[ -e T1R_Regions.tsv ] && rm T1R_Regions.tsv
for i in `cat T1R_best_hits.txt` ; do grep "$i" Blast_nonoverlapping.tsv >> T1R_Regions.tsv ; done


#Extend all best hits by 10000bp upstream and downstream . Result file : Potential_T1R_regions.tsv
Rscript $scripts_location/Rscript_merge_filter_extend_blast_hit_7avril.R $maximum_intron_length


#Split the T1R database and launch exonerate with these sequences against potential T1R regions (max intron length : 30000bp)

mkdir Splitted_db
$scripts_location/exonerate-2.2.0-x86_64/bin/fastasplit -f $T1R_database -c 28 --output Splitted_db
...
################################################################################################################################################
################################################################################################################################################
################## Search for T1R genes in a loop  #############################################################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
...
[ -e No_T1R_genes_coordinates.txt ] && rm No_T1R_genes_coordinates.txt
[ -e Frameshift_less_Pseudogenes.fa ] && rm Frameshift_less_Pseudogenes.fa

#Start the loop to search for T1R genes
...
number_regions_blast=`grep "[0-9]" Potential_T1R_regions.tsv | wc -l`
...
while [ "$current_nb_sequences" -gt "$previous_iteration_nb_sequences" ] && [ "$number_regions_blast" -gt "0" ] ; do
	...
	xargs samtools faidx $genome < Potential_T1R_regions.tsv > Potential_T1R_regions.fa

	mkdir Exonerate_raw_results_folder
	


	#If user dont have slurm, use nohup and wait. 
	
	for i in Splitted_db/* ; do
		file_name=`echo $i | sed 's/Splitted_db\///g'`
		sbatch -W -c 4 --qos=6hours --wrap="$scripts_location/exonerate-2.2.0-x86_64/bin/exonerate -E True --showtargetgff TRUE --model protein2genome --minintron 50 --maxintron $maximum_intron_length --ryo '%tcs' Splitted_db/$file_name Potential_T1R_regions.fa > Exonerate_raw_results_folder/$file_name.exo.rslt ; sleep 10" &
	done
	...
	#Perform the blastp
	blastp -query List_exonerate_cds.prot -db $scripts_location/Database_T1R_vertebrates.prot -outfmt '6 qseqid sseqid evalue' -out all_blastp.txt -max_target_seqs 1 -num_threads $number_of_thread
	...
	if [ "$nb_row_parsed_exonerate" -gt "0" ] ; then
		...
		for line in `cat Correct_coordinates_for_exonerate.tsv` ; do 
			...
			samtools faidx $scripts_location/Database_T1R_vertebrates.prot $best_query > query.prot
		...
		for file in Genes_predictions/*.exonerate ; do
			...
			#Test if the predicted gene is a T1R gene or not
			...
			if grep -q -i "T1R-Receptor\|Taste-receptor" blastp_result ; then
				...
				#file=Genes_predictions/NC_019879.2-28438421-28440954.exonerate 
					...
					getorf -sequence Complete_extanded_sequence.fa -outseq Filtered_predictions/$file_name_reduced.ORF -minsize 2200 -find 3
					...
					#If not ORF found, then determinate the gene state
						...
						query_total_length=`grep -m1 "$query_name" $scripts_location/Database_T1R_vertebrates.prot.fai | cut -f2`
						...
						makeblastdb -in Verification_scaffold.fa -dbtype nucl
						number_blast_hit=`tblastn -query predicted_cds.prot -db Verification_scaffold.fa -evalue 1e-10 -outfmt 6 | awk '{ if ($4 >= 30) { print } }' | wc -l`
				...
				#Lets make the same steps with slight modifications for the + strand
					...
					sed -i '1 i\>Complete_seq' Complete_extanded_sequence.fa
					getorf -sequence Complete_extanded_sequence.fa -outseq Filtered_predictions/$file_name_reduced.ORF -minsize 2200 -find 3 -reverse FALSE
			...
			else echo "$initial_header" >> No_T1R_genes_coordinates.txt
	...
	sed 's/-/	/g' No_T1R_genes_coordinates.txt >> Coordinates_already_examined.tsv
	...
	#re-process blast result to find potential T1R regions exclusing already found genes
	
	Rscript $scripts_location/Rscript_merge_filter_extend_blast_hit_Second_7avril.R $maximum_intron_length
	...
	if test -f "Potential_T1R_regions.tsv" ; then number_regions_blast=`grep "[0-9]" Potential_T1R_regions.tsv | wc -l` ; else number_regions_blast=0 ; fi
...
################################################################################################################################################
################################################################################################################################################
################## Search for remaining T1R genes with size a bit below  #######################################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
...
################################################################################################################################################
################################################################################################################################################
################## Final round to find T1R pseudogenes with length below previous iteration  ###################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
################################################################################################################################################
...
#First lets verify our genes with a phylogenetic tree rooted with T1R and CasR genes
...
if [ "$nb_seq" -gt "0" ] ; then
	mafft --add Frameshift_less_Pseudogenes_length.fa --keeplength $scripts_location/Database_T1R_vertebrates_plus_V2R_Casr.aln > ALL_verification_alignment.aln
else
	cp $scripts_location/Database_T1R_vertebrates_plus_V2R_Casr.aln ./ALL_verification_alignment.aln
fi
...
iqtree -s Final_ALL_verification_alignment.aln -st AA -nt $number_of_thread -m JTT+F+I+G4 -redo -n 400


#parse the tree

cp $scripts_location/Known_T1R_id.txt ./
cp $scripts_location/Known_outgroups_id.txt ./


Rscript $scripts_location/Tree_parser.R #This script keep only genes clustering with known T1R genes : Current_species_T1R.txt


grep "FALSE\|TRUE" Current_species_T1R.txt  > T1R_pseudos.id
grep -v "FALSE\|TRUE" Current_species_T1R.txt  > T1R_functionnal.id


xargs samtools faidx Potential_multiple_exon_CDS.fa < T1R_functionnal.id > Functionnal_T1R_genes.fa
xargs samtools faidx Pseudogenes_multiple_exon_reformat.fa < T1R_pseudos.id > Pseudogenes_T1R_genes.fa
...
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' Functionnal_T1R_genes.fa | sed 's/\*$//g' | awk -F '\t'  '!($2 ~ /\N/)' | tr "\t" "\n" > clear_Functionnal_T1R_genes.fa
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' Pseudogenes_T1R_genes.fa | sed 's/\*$//g' | awk -F '\t'  '!($2 ~ /\N/)' | tr "\t" "\n" > clear_Pseudogenes_T1R_genes.fa
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' Functionnal_T1R_genes.fa | sed 's/\*$//g' | awk -F '\t'  '($2 ~ /\N/)' | tr "\t" "\n" > unclear_Functionnal_T1R_genes.fa
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' Pseudogenes_T1R_genes.fa | sed 's/\*$//g' | awk -F '\t'  '($2 ~ /\N/)' | tr "\t" "\n" > unclear_Pseudogenes_T1R_genes.fa
cat unclear_Functionnal_T1R_genes.fa unclear_Pseudogenes_T1R_genes.fa > FINAL_Ambigous_T1R.fa
...
transeq clear_Functionnal_T1R_genes.fa clear_Functionnal_T1R_genes.prot ; sed -i 's/_1$//g' clear_Functionnal_T1R_genes.prot #translate CDS
perl $scripts_location/phobius/phobius.pl -long clear_Functionnal_T1R_genes.prot > Phobius_verification.txt #run phonius in long mode
grep ">" clear_Functionnal_T1R_genes.prot | sed 's/>//g' > gene_id.txt #extract cds id
...
$scripts_location/tmhmm-2.0c/bin/tmhmm clear_Functionnal_T1R_genes.prot > tmhmm_verification.txt
...
perl $scripts_location/rename_fasta.pl renaming_file_tm.txt clear_Functionnal_T1R_genes.fa > temporary.fasta ; mv temporary.fasta clear_Functionnal_T1R_genes.fa


# If two genes/pseudogenes are overlapping due to the multiple extensions, then keep only the longest found gene/pseudogene


awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' FINAL_Ambigous_T1R.fa | sort -t $'\t' -k1,1 -u | tr "\t" "\n" > FINAL_Ambigous_T1R_uniq.fa
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' clear_Pseudogenes_T1R_genes.fa | sort -t $'\t' -k1,1 -u | tr "\t" "\n" > clear_Pseudogenes_T1R_genes_uniq.fa
awk '/^>/ {printf("%s%s\t",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' clear_Functionnal_T1R_genes.fa | sort -t $'\t' -k1,1 -u | tr "\t" "\n" > clear_Functionnal_T1R_genes_uniq.fa



nb_seq=`grep -c ">" FINAL_Ambigous_T1R_uniq.fa`
if [ "$nb_seq" -gt "0" ] ; then
	grep ">" FINAL_Ambigous_T1R_uniq.fa | sed 's/>//g' | sed 's/-/	/g' | cut -f1,2,3 > Coordinates_ambigous_final.tsv
fi

nb_seq=`grep -c ">" clear_Pseudogenes_T1R_genes_uniq.fa`
if [ "$nb_seq" -gt "0" ] ; then
	grep ">" clear_Pseudogenes_T1R_genes_uniq.fa | sed 's/>//g' | sed 's/-/	/g' | cut -f1,2,3 > Coordinates_pseudogenes_final.tsv
fi

nb_seq=`grep -c ">" clear_Functionnal_T1R_genes_uniq.fa`
if [ "$nb_seq" -gt "0" ] ; then
	grep ">" clear_Functionnal_T1R_genes_uniq.fa | sed 's/>//g' | sed 's/-/	/g' | cut -f1,2,3 > Coordinates_genes_final.tsv
fi
...
for line in `cat best_genes_functionnal.tsv` ; do scaff=`echo "$line" | cut -f1` ; start=`echo "$line" | cut -f2` ; end=`echo "$line" | cut -f3` ; grep -m1 "$scaff.*$start.*$end" clear_Functionnal_T1R_genes_uniq.fa | sed 's/>//g' >> functionnal_to_keep.txt ; done
for line in `cat best_genes_ambigous.tsv` ; do scaff=`echo "$line" | cut -f1` ; start=`echo "$line" | cut -f2` ; end=`echo "$line" | cut -f3` ; grep -m1 "$scaff.*$start.*$end" FINAL_Ambigous_T1R_uniq.fa | sed 's/>//g' >> ambigous_to_keep.txt ; done
for line in `cat best_genes_pseudogenes.tsv` ; do scaff=`echo "$line" | cut -f1` ; start=`echo "$line" | cut -f2` ; end=`echo "$line" | cut -f3` ; grep -m1 "$scaff.*$start.*$end" clear_Pseudogenes_T1R_genes_uniq.fa | sed 's/>//g' >> pseudogenes_to_keep.txt ; done




if test -f "functionnal_to_keep.txt"  ; then xargs samtools faidx clear_Functionnal_T1R_genes_uniq.fa < functionnal_to_keep.txt > clear_Functionnal_T1R_genes.fa ; else echo "" > clear_Functionnal_T1R_genes.fa ; fi
if test -f "ambigous_to_keep.txt" ; then xargs samtools faidx FINAL_Ambigous_T1R_uniq.fa < ambigous_to_keep.txt > FINAL_Ambigous_T1R.fa ; else echo "" > FINAL_Ambigous_T1R.fa ; fi 
if test -f "pseudogenes_to_keep.txt" ; then xargs samtools faidx clear_Pseudogenes_T1R_genes_uniq.fa < pseudogenes_to_keep.txt > clear_Pseudogenes_T1R_genes.fa ; else echo "" > clear_Pseudogenes_T1R_genes.fa ; fi 
...
if [ $tm_filter == "TRUE" ] ; then
	...
	grep ">" clear_Functionnal_T1R_genes.fa | grep "phobius\|tmhmm" | sed 's/>//g' > 7tm_genes
	grep ">" clear_Functionnal_T1R_genes.fa | grep -v "phobius\|tmhmm" | sed 's/>//g' > non_7tm_genes
	xargs samtools faidx clear_Functionnal_T1R_genes.fa < 7tm_genes > FINAL_Functionnal_T1R_7tm.fa 
	xargs samtools faidx clear_Functionnal_T1R_genes.fa < non_7tm_genes >> clear_Pseudogenes_T1R_genes.fa
	mv clear_Pseudogenes_T1R_genes.fa FINAL_Pseudogenes_T1R.fa

else
	cp clear_Functionnal_T1R_genes.fa FINAL_Functionnal_T1R_7tm.fa 
	cp clear_Pseudogenes_T1R_genes.fa FINAL_Pseudogenes_T1R.fa
fi
...
transeq FINAL_Functionnal_T1R_7tm.fa FINAL_Functionnal_T1R_7tm.prot ; sed -i 's/_1$//g' FINAL_Functionnal_T1R_7tm.prot
blastp -query FINAL_Functionnal_T1R_7tm.prot -db $blast_database -outfmt '6 qseqid sseqid evalue' -out blastp_result_custom -max_target_seqs 1 -num_threads $number_of_thread
grep "T1R\|Taste-receptor" blastp_result_custom | cut -f1 | sort | uniq  > good_sequences
xargs samtools faidx FINAL_Functionnal_T1R_7tm.fa < good_sequences > temp.fa
mv temp.fa FINAL_Functionnal_T1R_7tm.fa ; rm good_sequences ; rm *.fai


blastx -query FINAL_Pseudogenes_T1R.fa -db $blast_database -outfmt '6 qseqid sseqid evalue' -out blastx_result_custom -max_target_seqs 1 -num_threads $number_of_thread
grep "T1R\|Taste-receptor" blastx_result_custom | cut -f1 | sort | uniq  > good_sequences_p
xargs samtools faidx FINAL_Pseudogenes_T1R.fa < good_sequences_p > temp_p.fa
mv temp_p.fa FINAL_Pseudogenes_T1R.fa ; rm good_sequences_p ; rm *.fai

blastx -query FINAL_Ambigous_T1R.fa -db $blast_database -outfmt '6 qseqid sseqid evalue' -out blastx_result_custom -max_target_seqs 1 -num_threads $number_of_thread
grep "T1R\|Taste-receptor" blastx_result_custom | cut -f1 | sort | uniq  > good_sequences_p
xargs samtools faidx FINAL_Ambigous_T1R.fa < good_sequences_p > temp_p.fa
mv temp_p.fa FINAL_Ambigous_T1R.fa ; rm good_sequences_p ; rm *.fai


#FINAL RESULTS FILES:
#FINAL_Functionnal_T1R_7tm.fa
#FINAL_Ambigous_T1R.fa
#FINAL_Pseudogenes_T1R.fa

#grep -c ">" clear_Functionnal_T1R_genes_cdhit.fa ; grep -c ">" clear_Pseudogenes_T1R_genes_cdhit.fa ; grep -c ">" FINAL_Ambigous_T1R.fa
...
nb_functionnal=`grep -c ">" FINAL_Functionnal_T1R_7tm.fa`
nb_pseudo_edge=`grep -c ">" FINAL_Pseudogenes_T1R.fa`
nb_ambigous=`grep -c ">" FINAL_Ambigous_T1R.fa`

echo "Search of T1R is finished. There are $nb_functionnal potentially functionnal genes, $nb_pseudo_edge pseudogenes or fragments and $nb_ambigous ambigous sequences"
