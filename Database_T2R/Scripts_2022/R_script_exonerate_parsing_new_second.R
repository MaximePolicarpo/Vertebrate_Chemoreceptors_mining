...
regions_functionnals_or <- read.table("Coordinates_Functionnal_T2Rs.txt", header=FALSE, sep="\t")
colnames(regions_functionnals_or) <- c("scaffold", "start", "end")


#Load multiple exon regions and rename columns
regions_multiple_exon_or <- read.table("Coordinates_MulitpleExon_T2Rs.txt", header=FALSE, sep="\t")
colnames(regions_multiple_exon_or) <- c("scaffold", "start", "end")
...
for (row in 1:nrow(regions_multiple_exon_or)) {

  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(regions_multiple_exon_or[row, "scaffold"]) & start <= (regions_multiple_exon_or[row, "start"]) & end >= (regions_multiple_exon_or[row, "start"])))
  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(regions_multiple_exon_or[row, "scaffold"]) & start >= (regions_multiple_exon_or[row, "start"]) & end <= (regions_multiple_exon_or[row, "end"])))
  exonerate_rslt <- exonerate_rslt %>% filter(!(seqnames == as.character(regions_multiple_exon_or[row, "scaffold"]) & start <= (regions_multiple_exon_or[row, "end"]) & end >= (regions_multiple_exon_or[row, "end"])))
...
for(putative_protein_length in c(250,240,210,180,150)){ 
...
if (nrow(Best_hits_filtered) > 0) {
	...
	Best_hits_filtered <- Best_hits_filtered %>% mutate(extanded_start = start-500) %>% mutate(extanded_end = end+500)
