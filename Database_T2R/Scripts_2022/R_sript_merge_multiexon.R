...
blast_rslt <- read.table("tblastn_functionnal_and_known_t2r_vs_genome.tblastn", header=FALSE, sep="\t")
...
regions_functionnals_t2r <- read.table("Coordinates_Functionnal_T2Rs.txt", header=FALSE, sep="\t")

#name columns
colnames(regions_functionnals_t2r) <- c("scaffold", "start", "end")
...
for (row in 1:nrow(regions_functionnals_t2r)) {

  Best_hits_filtered <- Best_hits_filtered %>% filter(!(seqnames == as.character(regions_functionnals_t2r[row, "scaffold"]) & start <= (regions_functionnals_t2r[row, "start"]) & end >= (regions_functionnals_t2r[row, "start"])))
  Best_hits_filtered <- Best_hits_filtered %>% filter(!(seqnames == as.character(regions_functionnals_t2r[row, "scaffold"]) & start >= (regions_functionnals_t2r[row, "start"]) & end <= (regions_functionnals_t2r[row, "end"])))
  Best_hits_filtered <- Best_hits_filtered %>% filter(!(seqnames == as.character(regions_functionnals_t2r[row, "scaffold"]) & start <= (regions_functionnals_t2r[row, "end"]) & end >= (regions_functionnals_t2r[row, "end"])))
...
for (i in seq(1:nrow(Best_hits_filtered))){
	...
	if (nrow(regions_functionnals_t2r %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-25000) & start < curr_start)) > 0){

		new_curr_start = (tail(regions_functionnals_t2r %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-25000) & start < curr_start) %>% dplyr::arrange(end), 1)$end) + 50

	} else new_curr_start = curr_start-25000


	if (nrow(regions_functionnals_t2r %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+25000) & end > curr_end)) > 0){

		new_curr_end = (head(regions_functionnals_t2r %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+25000) & end > curr_end) %>% dplyr::arrange(start), 1)$start) - 50
