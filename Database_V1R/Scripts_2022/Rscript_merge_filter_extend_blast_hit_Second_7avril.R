...
blast_rslt <- read.table("V1R_vs_Genome.blastn", header=FALSE, sep="\t")
...
V1R_best_hits <- scan(file="V1R_Regions.tsv", what="character")


Best_hits_filtered <- Best_hits_filtered %>% filter(samtools_name %in% V1R_best_hits)


#remove regions of already found V1R genes
...
if (nrow(Best_hits_filtered) > 0) {
  ...
  for (i in seq(1:nrow(Best_hits_filtered))){
    ...
    if (nrow(found_genes %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-10000) & start < curr_start)) > 0){
  
      new_curr_start = (tail(found_genes %>% filter(scaffold == curr_seqnames) %>% filter(end >= (curr_start-10000) & start < curr_start) %>% dplyr::arrange(end), 1)$end) + 50
  
    } else new_curr_start = curr_start-10000
  
  
    if (nrow(found_genes %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+10000) & end > curr_end)) > 0){
  
      new_curr_end = (head(found_genes %>% filter(scaffold == curr_seqnames) %>% filter(start <= (curr_end+10000) & end > curr_end) %>% dplyr::arrange(start), 1)$start) - 50
  
    } else new_curr_end = curr_end+10000
...
write(Best_hits_filtered$samtools_name, file="Potential_V1R_regions.tsv")
