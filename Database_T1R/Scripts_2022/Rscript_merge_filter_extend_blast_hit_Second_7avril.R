...
blast_rslt <- read.table("T1R_vs_Genome.blastn", header=FALSE, sep="\t")
...
T1R_best_hits <- scan(file="T1R_Regions.tsv", what="character")


Best_hits_filtered <- Best_hits_filtered %>% filter(samtools_name %in% T1R_best_hits)
...
write(Best_hits_filtered$samtools_name, file="Potential_T1R_regions.tsv")
